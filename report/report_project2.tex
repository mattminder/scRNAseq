...
\par
The neural network consists of fully connected layers and ReLU-activations in between them. It classifies into two classes (non-stem or stem cell) and returns the probabilities using a softmax classifier. Six layers were used for the dimensionality reduced data. Due to a lack of working memory, a hidden layer fc0 with 500 nodes was added at the front of the network. This allows us to compare the addition of a layer with 500 nodes to a PCA to 500 dimensions. The architecture can be found in Table \ref{tab:ArchitectureNN}.
