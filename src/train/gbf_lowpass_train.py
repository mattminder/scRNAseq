...
def do_gbf_lowpass_train(train_x, train_y):
    """
    Trains classifier based on low-pass GFS transformed data (for historical reasons here called gbf)
    :param train_x: Train set
    :param train_y: Train response
    :return: Nothing
    """
    CLASSIF_FOLDER = '../../res/classif/gbf_lowpass/'
    NETWORK_FOLDER = '../../src/network/'

    # Transforming Data
    print('Transforming Data')
    transf = fit_networkPCA(train_x, CLASSIF_FOLDER, ret=True, network_folder=NETWORK_FOLDER,
                            method='gbf', attenuation='low-pass',
