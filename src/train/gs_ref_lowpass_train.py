...
def do_gs_ref_lowpass_train(train_x, train_y):
    """
    Trains classifier on low-pass GS transformed data
    :param train_x: Train set
    :param train_y: Train response
    :return:
    """
    CLASSIF_FOLDER = '../../res/classif/gs-ref_lowpass/'
    NETWORK_FOLDER = '../../src/network/'

    # Transforming Data
    print('Transforming Data')
    transf = fit_networkPCA(train_x, CLASSIF_FOLDER, ret=True, network_folder=NETWORK_FOLDER,
                            method='gs-ref', attenuation='low-pass',
