...
#assert((cell_names_x == cell_names_y).all())


# Preprocessing
scaled_x = prep.scale(train_x)
...
# Save Classif
savefile = open(RES_FOLDER + 'log_lasso_classif.txt', 'wb')
...
herring_scaled = prep.scale(herring_x)
...
joost_scaled = prep.scale(joost_x)

# Prediction
print('Predictions')
herring_pred = classif.predict_proba(herring_scaled)
joost_pred = classif.predict_proba(joost_scaled)
np.save(arr=herring_pred, file=RES_FOLDER+'log_lasso_preds_herring.npy')
np.save(arr=joost_pred, file=RES_FOLDER+'log_lasso_preds_joost.npy')
