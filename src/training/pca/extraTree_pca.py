...
print('Fitting Model')
classif = ens.ExtraTreesClassifier(random_state=161,
                                   n_jobs=2,
                                   verbose=1,
                                   n_estimators=5000
                                   ).fit(train_x,
...
# Save Classif
savefile = open(RES_FOLDER + 'extraTree_' + str(n_pcs) + 'pcs_classif.txt', 'wb')
...
np.save(arr=herring_pred, file=RES_FOLDER+'extraTree_' + str(n_pcs) + 'pcs_preds_herring.npy')
np.save(arr=joost_pred, file=RES_FOLDER+'extraTree_' + str(n_pcs) + 'pcs_preds_joost.npy')
